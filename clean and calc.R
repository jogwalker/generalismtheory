...
##### redo host traits from definitive subset only
host.traits <- dat.ph.def %>% dplyr::select(GEO,MaxL,new_hname) %>% group_by(new_hname,GEO) %>% summarize(maxL=max(MaxL,na.rm=T))
...
# complexNA <- par.traits[which(is.na(par.traits$Complex)),]
# # Host_stage not accurate because only the first line if more than one host. Not a parasite trait...
# par.traits <- dplyr::select(par.traits,-Host_stage)
save(par.traits,file="~/dat/generalismtheory/par.traits30may.RData")
...
hp <- dat.ph.def %>% dplyr::select(new_pname,new_hname,GEO) %>% distinct()
...
hp.def.geo <- hp.traits %>% group_by(new_pname,GEO,P_Taxon,Endoparasite,new_Complex,Trophic,Horizontal) %>% summarize(meanMaxL=mean(maxL,na.rm=T),sdMaxL=sd(maxL,na.rm=T),maxMaxL=max(maxL,na.rm=T),n=sum(!is.na(maxL)))

hp.traits2 <- hp.traits %>% dplyr::select(-GEO) %>% distinct()
hp.def.nogeo <- hp.traits2 %>% group_by(new_pname,P_Taxon,Endoparasite,new_Complex,Trophic,Horizontal) %>% summarize(meanMaxL=mean(maxL,na.rm=T),sdMaxL=sd(maxL,na.rm=T),maxMaxL=max(maxL,na.rm=T),n=sum(!is.na(maxL)))
