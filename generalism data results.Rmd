...
### calculate indices - and host traits at the same time??
...
load('160517_generalism_keyObj.RData') # tree called pgd_final

############
# FUNCTION
###########

calcMetrics <- function(assoc,paralist,hostlist,ro_allhost_tree,filename) {
  cdmat <- makecdmat(assoc,paralist,hostlist)
  pd_allhost <- pd(cdmat,ro_allhost_tree)
  save(pd_allhost,file=paste("pd",filename,sep="_"))
  gen <- calcother(assoc)
  generalism <- full_join(pd_allhost,gen,by="pname")
}

makecdmat <- function(assoc,paralist,hostlist) {
...
###### CONTINUE FROM HERE
calcother <- function(assoc) {
  
  generalism <- assoc %>% group_by(new_pname,P_Taxon,Endoparasite,Complex,Trophic,Horizontal) %>% summarize(hostN=n(),edges=(n()*(n()-1))/2,meanMaxL=mean(maxL,na.rm=T),sdMaxL=sd(maxL,na.rm=T),maxMaxL=max(maxL,na.rm=T)) # maybe doesn't make sense to do it this way for PS (sum of pgd)
  # write another function that takes the hlist and sums in pgd_final
  
  
  
  generalism<-data.frame(pname=character(), hostN=integer(), edges=integer(), sumPGD=numeric(), SPD=numeric(), sSPD=numeric(), meanMaxL=numeric(),maxMaxL=numeric(),cvMaxL=numeric(), stringsAsFactors=FALSE)
  ...
  for(paras in 1:length(paralist)){
    ...
    pname<-as.character(paralist[paras])
	  ...
	  maxMaxL <- 
	  cvMaxL <- 
	  generalism[paras,]<-c(pname, hostN, edges, sumPGD, SPD, sSPD)
  }
  save()
}
