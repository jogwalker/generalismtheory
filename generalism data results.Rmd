...
write.csv(generalism2,file="~/dat/generalismtheory/nogeo.csv")
...
```{r newanalysis,echo=FALSE,eval=TRUE,results="hide"}
# October 17 for revision ************************************************************************************************************
hp.geoDirect2$Transmission <- "Direct"
hp.geoTrophic$Transmission <- "Trophic"
alldat <- bind_rows(hp.geoDirect2,hp.geoTrophic)
alldat$scalemeanmaxL <- scale(alldat$meanMaxL)
alldat$scalemaxmaxL <- scale(alldat$maxMaxL)

degreemod <- glm.nb(data=alldat,degree ~ scalemeanmaxL*Transmission*GeoGroup)
Gmod <- glm(data=alldat,G ~ scalemeanmaxL*Transmission*GeoGroup,family="binomial")
SPDmod <- lm(data=alldat,SPD ~ scalemeanmaxL*Transmission*GeoGroup)
SESPDmod <- lm(data=alldat,pd.obs.z ~ scalemeanmaxL*Transmission*GeoGroup)

degreemod2 <- glm.nb(data=alldat,degree ~ scalemaxmaxL*Transmission*GeoGroup)
Gmod2 <- glm(data=alldat,G ~ scalemaxmaxL*Transmission*GeoGroup,family="binomial")
SPDmod2 <- lm(data=alldat,SPD ~ scalemaxmaxL*Transmission*GeoGroup)
SESPDmod2 <- lm(data=alldat,pd.obs.z ~ scalemaxmaxL*Transmission*GeoGroup)

degreemod3 <- glm.nb(data=alldat,degree ~ cvmaxL*Transmission*GeoGroup)
#Gmod3 <- glm(data=alldat,G ~ cvmaxL*Transmission*GeoGroup,family="binomial") # algorithm did not converge because this one doesn't exist
SPDmod3 <- lm(data=alldat,SPD ~ cvmaxL*Transmission*GeoGroup)
SESPDmod3 <- lm(data=alldat,pd.obs.z ~ cvmaxL*Transmission*GeoGroup)

alltables <- data.frame(NULL)
l <- list(degreemod,degreemod2,degreemod3,Gmod,Gmod2,SPDmod,SPDmod2,SPDmod3,SESPDmod,SESPDmod2,SESPDmod3)
lname <- c("degreemod","degreemod2","degreemod3","Gmod","Gmod2","SPDmod","SPDmod2","SPDmod3","SESPDmod","SESPDmod2","SESPDmod3")
for (i in 1:length(l)) {
  mod <- l[[i]]
  tab <- bind_cols(data.frame(round(coef(summary(mod))[,1],digits=3)),data.frame(round(confint(mod),digits=3)), data.frame(round(coef(summary(mod))[,3],digits=3)),data.frame(rep(mod$df.residual,8)),data.frame(rep(as.character(lname[i]),8)))
  alltables <- bind_rows(alltables,tab)
}
names(alltables) <- c("coef", "low.ci","high.ci","z","df","mod")
write.csv(alltables,file="multivartables.csv")
...
write.csv(hp.geoDirect,file="~/dat/generalismtheory/geo.csv")
...
# ggplot(data=hp.direct, aes(x=degree,fill=Endoparasite)) + geom_histogram()
# table(hp.direct$Endoparasite,hp.direct$degree <= 4)
```
